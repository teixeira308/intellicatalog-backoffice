...
const NovoProduto = () => { 
  ...
  useEffect(() => {
      const fetchCategorias = async () => {
        try {
          const data = await getCategoria(categoria);
          setCategorias(data); 
          console.log(data)
        } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createProduto(filteredData, categoria);
    ...
    <C.Container>
      ...
      <Container maxWidth="md" sx={{ textAlign: "center" }}>
        ...
        <C.ModalForm onSubmit={handleSubmit}>
                  <Typography variant="subtitle1" mb={2}>Categoria: {categorias.name}</Typography>
